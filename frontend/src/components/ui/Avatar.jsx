...
const Avatar = ({ name, id, src, avatar, avatar_url, size = "md", className, title }) => {
  const imageSrc = src || avatar || avatar_url;
  return (
    <div
      title={title || name}
      className={cn(
      ...
      style={{ backgroundColor: imageSrc ? undefined : colorFromId(id || name || "") }}
    >
      {imageSrc ? <img src={imageSrc} alt={name} className="h-full w-full object-cover" /> : initials(name)}
    </div>
  );
};
...
export const AvatarStack = ({ users = [], max = 4, size = "sm" }) => {
  ...
  return (
    <div className="flex items-center -space-x-2">
      {shown.map((u) => (
        <Avatar key={u.id} id={u.id} name={u.name} src={u.avatar || u.avatar_url} size={size} />
      ))}
      {extra > 0 && (
