      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
            ...
            >
              {/* Header */}
              <div className="p-5 border-b border-slate-800/80 bg-slate-900/80 backdrop-blur flex items-center justify-between">
              ...
              {/* Filter Tabs */}
              <div className="px-5 py-3.5 flex gap-2 border-b border-slate-800/80 bg-slate-900/50 text-xs overflow-x-auto no-scrollbar">
                <button
              ...
              <div className="flex-1 overflow-y-auto p-5 relative">
                ...
                {activities.length > 0 && (
                  <div className="absolute left-[2.4rem] top-7 bottom-7 w-0.5 bg-slate-800/80 rounded-full" />
                )}
                ...
                {loading && activities.length === 0 ? (
                  <div className="space-y-4">
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        key={i}
                        className="flex items-start gap-4 p-3.5 rounded-xl bg-slate-800/30 border border-slate-800/50 animate-pulse"
                      >
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center p-8">
                    <div className="w-12 h-12 rounded-2xl bg-slate-800/60 text-slate-500 flex items-center justify-center mb-3 border border-slate-800">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="relative shrink-0 z-10">
                            <div className="w-9 h-9 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs font-semibold ring-2 ring-slate-900 shadow-md overflow-hidden">
                              {item.actorAvatar ? (
                                <img
                            ...
                            <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-slate-900 border border-slate-700 flex items-center justify-center">
                              <IconComponent className="w-2.5 h-2.5 text-indigo-400" />
                            </div>
                          </div>

                          {/* Activity Content Card */}
                          <div className="flex-1 min-w-0 bg-slate-800/40 hover:bg-slate-800/70 border border-slate-800 hover:border-slate-700/80 rounded-xl p-3.5 transition-all shadow-sm group-hover:shadow-md">
                ...
                {hasMore && (
                  <div className="pt-6 pb-2 text-center">
                    <button
                      ...
                      disabled={loading}
                      className="inline-flex items-center gap-2 px-4 py-2 text-xs font-medium text-indigo-400 border border-indigo-500/30 rounded-xl hover:bg-indigo-500/10 transition-all disabled:opacity-50"
                    >
                      {loading ? (
                        <>
